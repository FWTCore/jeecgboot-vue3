...
<script lang="ts" name="mzx-employee-salary" setup>
  //ts语法
  ...
  const { tableContext } = useListPage({
    designScope: 'employee-salary-page',
    tableProps: {
      title: '员工薪资列表',
      api: employeeSalaryList,
      size: 'small',
      columns: salaryColumns,
      showTableSetting: false,
      formConfig: {
        schemas: searchSalaryColumns,
  ...
  async function handleDelete(record) {
    await employeeSalaryDelete({ id: record.id }, reload);
  }
  /**
   * 批量删除事件
   */
  async function batchHandleDelete() {
    await employeeSalaryBathDelete({ ids: selectedRowKeys.value }, () => {
